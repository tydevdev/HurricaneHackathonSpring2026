...
| Click-driven tab shuffle | At stage 2+, click a different app tab | App tabs reshuffle into a new order immediately after the page switch. The Landing escape link stays fixed before the shuffled app tabs | 2-5 | implemented |
...
| Stacked page warp lottery | At stage 3+, switch to a different app tab | The new page content has a 20%/30%/40% overall chance at stages 3/4/5 to pick one recoverable warp, with uncommon two-mode stacks and rare three-mode stacks. Possible modes: color invert, upside down, zero gravity, melt, mirror, duplicate echo, font infection, jelly, translation failure, loose screws, 404 bleed, and autofill hallucination. If any hit, Helpy appears bottom-right with a one-click fix that clears only the warp | 3-5 | implemented |
| Page fracture overlay | Reach stage 5, then wait at least 30 seconds and clear the 120-second crack cooldown | Cracks spread across the page and paper-like UI shards break loose and fall off screen; earlier stages never crack, and tab switches inside the entry delay or cooldown do not replay the crack experience | 5 | implemented |
| Helpy Spackle repair | Reach stage 5 where page fractures are visible | Helpy pops up in a bottom-center card with an integrated progress bar and brush button; pressing and dragging the brush for 2 seconds fills the bar, removes only the cracks, leaves the current decay stage intact, and respects the shared 120-second crack cooldown before fractures can return | 5 | implemented |
| Human dev trivia rescue | Reach stage 5 and wait 30-60 seconds | A bottom-right real-life developer bubble arrives on a randomized delay; a watching eye pressures the user to click a large red Ignore button above the dev, which dismisses him for another 30-60 seconds, while each rescue appearance randomly draws three multiple-choice questions from a 30-question pool; three correct answers reset the shared decay score to phase 1, show a thank-you message, then dismiss the bubble | 5 | implemented |
